...
module.exports = {
    ...
    run: async (bot, messageOrInteraction, args, { prefixUsed, member, guild, reply }) => {
        return new Promise(async resolve => {
            if (!Utils.hasPermission(member, config.Suggestions.ManageSuggestionsRole)) {
                reply(Embed({ preset: "nopermission" }), {
                    ephemeral: true
                });
            ...
            if (!id) {
                reply(Embed({
                    preset: "error",
                    description: lang.AdminModule.Commands.Sdeny.NoID,
            ...
            if (suggestion) {
                ...
                reply(Embed({
                    title: lang.AdminModule.Commands.Sdeny.Denied
                }));
                ...
                reply(Embed({
                    preset: "error",
                    description: lang.AdminModule.Commands.Sdeny.InvalidID
                }), { ephemeral: true });
    ...
    description: "Change a suggestion to denied",
    aliases: ["sdenied"],
    usage: "sdeny <message ID/suggestion ID> [reason]",
    arguments: [
        {
            name: "id",
            description: "The ID of the suggestion or message",
            required: true,
            type: "STRING"
        },
        {
